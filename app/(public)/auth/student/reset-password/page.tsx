// app/(public)/auth/student/reset-password/page.tsx
'use client';
...
export default function StudentResetPassword() {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        const res = await fetch(`/api/auth/verify-reset-token?token=${token}&role=student`);
        if (res.ok) {
          setValidToken(true);
        } else {
          setValidToken(false);
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    ...
    try {
      const res = await fetch('/api/auth/reset-password', {
        ...
        body: JSON.stringify({ token, password: data.password, role: 'student' }),
      });
