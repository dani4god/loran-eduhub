// models/SelfPacedEnrollment.ts — full file
...
const SelfPacedEnrollmentSchema = new Schema<ISelfPacedEnrollment>(
  {
    ...
    lockedAtWeek: { type: Number },
    unlockedByTutorAt: { type: Date },
    completedAt: { type: Date },
    certificateId: { type: Schema.Types.ObjectId, ref: 'SelfPacedCertificate' },
  },
  { timestamps: true }
)

SelfPacedEnrollmentSchema.index({ selfPacedStudentId: 1, courseId: 1 }, { unique: true })

const SelfPacedEnrollment: Model<ISelfPacedEnrollment> =
  mongoose.models.SelfPacedEnrollment || mongoose.model<ISelfPacedEnrollment>('SelfPacedEnrollment', SelfPacedEnrollmentSchema)
