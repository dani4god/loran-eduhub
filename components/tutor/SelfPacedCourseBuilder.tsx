    ...
    <div className="pt-16 lg:pt-0 min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3 flex-wrap">
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} className="flex-1 min-w-[140px] font-bold text-gray-900 text-base outline-none" placeholder="Course title" />
      ...
      {activeTab === 'settings' && (
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-4">
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" min={0} value={price} onChange={(e) => setPrice(Number(e.target.value))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Category</label>
                <input value={category} onChange={(e) => setCategory(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
              </div>
            </div>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Course description (shown on public page)" className="w-full mt-3 border border-gray-200 rounded-lg px-3 py-2 text-sm" />
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-900 mb-3">
              <input type="checkbox" checked={coachingEnabled} onChange={(e) => setCoachingEnabled(e.target.checked)} /> Offer 1-on-1 Coaching
            </label>
            {coachingEnabled && (
              <div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-900 mb-3">
              <input type="checkbox" checked={discordEnabled} onChange={(e) => setDiscordEnabled(e.target.checked)} /> Show Discord Community Link
            </label>
            {discordEnabled && (
              <textarea value={discordDescription} onChange={(e) => setDiscordDescription(e.target.value)} rows={2} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            )}
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-900 mb-3">
              <input type="checkbox" checked={workshopEnabled} onChange={(e) => setWorkshopEnabled(e.target.checked)} /> Weekly Free Workshop (optional)
            </label>
            {workshopEnabled && (
              <div className="grid grid-cols-2 gap-3">
                <input value={workshopDay} onChange={(e) => setWorkshopDay(e.target.value)} placeholder="Day (e.g. Saturday)" className="border border-gray-200 rounded-lg px-3 py-2 text-sm" />
                <input value={workshopTime} onChange={(e) => setWorkshopTime(e.target.value)} placeholder="Time (e.g. 4:00 PM)" className="border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="border-2 border-dashed border-gray-200 rounded-lg p-3 text-center">
                ...
                <label className="text-xs font-semibold text-blue-600 cursor-pointer">
                  ...
                  <input type="file" accept="image/*" className="hidden" disabled={uploadingSig} onChange={(e) => e.target.files?.[0] && uploadFile(e.target.files[0], setCertSignature, setUploadingSig)} />
                </label>
              </div>
              <div className="border-2 border-dashed border-gray-200 rounded-lg p-3 text-center">
                ...
                <label className="text-xs font-semibold text-blue-600 cursor-pointer">
                  ...
                  <input type="file" accept="image/*" className="hidden" disabled={uploadingLogo} onChange={(e) => e.target.files?.[0] && uploadFile(e.target.files[0], setCertLogo, setUploadingLogo)} />
                </label>
              </div>
            </div>
          </div>
